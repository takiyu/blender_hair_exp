...
static int find_mface_for_root_position(const Span<MVert> vertices,
                                        const MFace *mface,
  ...
  for (const int possible_mface_i : possible_mface_indices) {
    const MFace &possible_mface = mface[possible_mface_i];
    {
      ...
      closest_on_tri_to_point_v3(point_in_triangle,
                                 root_pos,
                                 vertices[possible_mface.v1].co,
                                 vertices[possible_mface.v2].co,
                                 vertices[possible_mface.v3].co);
 ...
 */
static float4 compute_mface_weights_for_position(const Span<MVert> vertices,
                                                 const MFace &mface,
                                                 const float3 &position)
{
  ...
  if (mface.v4) {
    ...
    copy_v3_v3(mface_verts_su[0], vertices[mface.v1].co);
    copy_v3_v3(mface_verts_su[1], vertices[mface.v2].co);
    copy_v3_v3(mface_verts_su[2], vertices[mface.v3].co);
    copy_v3_v3(mface_verts_su[3], vertices[mface.v4].co);
    interp_weights_poly_v3(mface_weights, mface_verts_su, 4, position);
  }
  else {
    interp_weights_tri_v3(mface_weights,
                          vertices[mface.v1].co,
                          vertices[mface.v2].co,
                          vertices[mface.v3].co,
                          position);
                                      ...
                                      bool *r_could_not_convert_some_curves)
{
  ...
  const Span<MVert> vertices = bke::mesh_vertices(surface_me);
  ...
  for (const int new_hair_i : IndexRange(hair_num)) {
    ...
    const int mface_i = find_mface_for_root_position(
        vertices, mfaces, poly_to_mface_map[poly_i], root_pos_su);
    const MFace &mface = mfaces[mface_i];

    const float4 mface_weights = compute_mface_weights_for_position(vertices, mface, root_pos_su);
                                               ...
                                               bool *r_missing_uvs)
{
  ...
  const Span<MVert> vertices = bke::mesh_vertices(surface_mesh);
      ...
      threading::parallel_for(curves.curves_range(), 256, [&](const IndexRange curves_range) {
        for (const int curve_i : curves_range) {
          ...
          if (!surface_uv_map.is_empty()) {
            ...
            const float3 &p0_su = vertices[loops[corner0].v].co;
            const float3 &p1_su = vertices[loops[corner1].v].co;
            const float3 &p2_su = vertices[loops[corner2].v].co;
            float3 bary_coords;
    ...
    case AttachMode::Deform: {
      ...
      threading::parallel_for(curves.curves_range(), 256, [&](const IndexRange curves_range) {
        for (const int curve_i : curves_range) {
          ...
          const float3 &p0_su = vertices[loops[looptri.tri[0]].v].co;
          const float3 &p1_su = vertices[loops[looptri.tri[1]].v].co;
          const float3 &p2_su = vertices[loops[looptri.tri[2]].v].co;
