    ...
    bContext *C, uiLayout *layout, PointerRNA *ptr, PointerRNA *node_ptr, const char *text)
{
  ...
  switch (type) {
    ...
    case SOCK_IMAGE: {
      ...
      if (ELEM(node_tree->type, NTREE_GEOMETRY, NTREE_SIMULATION)) {
        if (text[0] == '\0') {
          uiTemplateID(layout,
                       C,
                       ptr,
                           ...
                           const bool selected)
{
  ...
  if (ELEM(snode.edittree->type, NTREE_GEOMETRY, NTREE_SIMULATION)) {
                    ...
                    const bool selected)
{
  ...
  if (ELEM(snode.edittree->type, NTREE_GEOMETRY, NTREE_SIMULATION) && !(link.flag & NODE_LINK_DRAGGED)) {
    if ((link.fromsock && link.fromsock->display_shape == SOCK_DISPLAY_SHAPE_DIAMOND) &&
