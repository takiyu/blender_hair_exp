...
static void updateDepsgraph(ModifierData *md, const ModifierUpdateDepsgraphContext *ctx)
{
  ...
  if (amd->start_cap != nullptr) {
    ...
    DEG_add_special_eval_flag(ctx->node, &amd->start_cap->id, DAG_EVAL_NEED_CPU_SUBSURF);
  }
  if (amd->end_cap != nullptr) {
    DEG_add_object_relation(
        ctx->node, amd->end_cap, DEG_OB_COMP_GEOMETRY, "Array Modifier End Cap");
    DEG_add_special_eval_flag(ctx->node, &amd->end_cap->id, DAG_EVAL_NEED_CPU_SUBSURF);
  }
  if (amd->curve_ob) {
    DEG_add_object_relation(
