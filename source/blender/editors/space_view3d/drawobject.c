    ...
    if (BKE_mesh_runtime_looptri_ensure(me)) {
      ...
      for (mp = polys, i = 0; i < mpoly_len; i++, mp++) {
        if (facemap_data[i] == facemap) {
          for (int j = 2; j < mp->totloop; j++) {
            copy_v3_v3(GPU_vertbuf_raw_step(&pos_step), positions[corner_verts[mlt->tri[0]]]);
            copy_v3_v3(GPU_vertbuf_raw_step(&pos_step), positions[corner_verts[mlt->tri[1]]]);
            copy_v3_v3(GPU_vertbuf_raw_step(&pos_step), positions[corner_verts[mlt->tri[2]]]);
            vbo_len_used += 3;
            mlt++;
          }
        }
      ...
      for (mp = polys, i = 0; i < mpoly_len; i++, mp++) {
        if (facemap_data[i] == facemap) {
          ...
          for (int j = 2; j < mp->totloop; j++) {
            copy_v3_v3(GPU_vertbuf_raw_step(&pos_step), positions[*corner_vert_start]);
            copy_v3_v3(GPU_vertbuf_raw_step(&pos_step), positions[*corner_vert_a]);
            copy_v3_v3(GPU_vertbuf_raw_step(&pos_step), positions[*corner_vert_b]);
            vbo_len_used += 3;
